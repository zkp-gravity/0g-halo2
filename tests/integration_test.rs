...
fn mock_proof_mnist_tiny() {
    let img = load_image(&PathBuf::from(IMG_PATH)).unwrap();
    let wnn = load_wnn(&PathBuf::from(MODEL_PATH_MNIST_TINY)).unwrap();
...
fn mock_proof_mnist_small() {
    let img = load_image(&PathBuf::from(IMG_PATH)).unwrap();
    let wnn = load_wnn(&PathBuf::from(MODEL_PATH_MNIST_SMALL)).unwrap();
